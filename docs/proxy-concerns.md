...
`PlayerRoute.kt` (`content/ui/src/main/java/com/opentune/content/ui/catalog/PlayerRoute.kt`) calls:
...
`EndpointClientRegistry.kt` (`app/src/main/java/com/opentune/app/providers/EndpointClientRegistry.kt`) `buildClient()` calls `buildHttpClient(entity.proxyId)`:
...
`PlaybackSpecExt.kt` (`player/src/main/java/com/opentune/player/engine/PlaybackSpecExt.kt`) `toMediaSource()`:
...
Sidecar subtitles get the same treatment in `SubtitleController.kt` (`player/src/main/java/com/opentune/player/controller/SubtitleController.kt`) `prepareWithSidecar()` — a parallel `SingleSampleMediaSource` is built with the same header interceptor.

## Path B: SMB (no HTTP URL of its own → local stream proxy)

`SmbProviderInstance.kt` (`content/providers/smb/src/main/java/com/opentune/smb/SmbProviderInstance.kt`) `getPlaybackSpec()`:
...
`StreamProxy.kt` (`server/src/main/java/com/opentune/server/StreamProxy.kt`) — Ktor CIO server on port 7920:
...
- `SmbPlaybackHooks` (`content/providers/smb/src/main/java/com/opentune/smb/SmbPlaybackHooks.kt`) revokes the token on disposal, closing the SMB session
...
| Concern | File |
|---|---|
| Player URL request | `content/ui/src/main/java/com/opentune/content/ui/catalog/PlayerRoute.kt` |
| PlaybackSpec data class | `player/src/main/java/com/opentune/player/PlaybackContracts.kt` |
| Playback engine | `player/src/main/java/com/opentune/player/engine/PlaybackEngine.kt` |
| Header injection for playback | `player/src/main/java/com/opentune/player/engine/PlaybackSpecExt.kt` |
| Subtitle header injection | `player/src/main/java/com/opentune/player/controller/SubtitleController.kt` |
| Provider contract | `content/contract/src/main/java/com/opentune/content/contract/ProviderContracts.kt` |
| Endpoint client registry (proxy wiring) | `app/src/main/java/com/opentune/app/providers/EndpointClientRegistry.kt` |
| Proxy repository | `content/ui/src/main/java/com/opentune/content/ui/providers/ProxyRepository.kt` |
| Proxy contract | `proxy/contract/src/main/java/com/opentune/proxy/contract/ProxyProvider.kt` |
| HTTP proxy provider | `proxy/providers/http/src/main/java/com/opentune/proxy/http/HttpProxyProvider.kt` |
| Stream registrar contract | `content/contract/src/main/java/com/opentune/content/contract/StreamRegistrar.kt` |
| Local stream proxy (Ktor server) | `server/src/main/java/com/opentune/server/StreamProxy.kt` |
| Server bootstrap | `server/src/main/java/com/opentune/server/OpenTuneServer.kt` |
| SMB provider instance | `content/providers/smb/src/main/java/com/opentune/smb/SmbProviderInstance.kt` |
| SMB playback hooks | `content/providers/smb/src/main/java/com/opentune/smb/SmbPlaybackHooks.kt` |
| JS provider instance | `content/providers/js/src/main/java/com/opentune/provider/js/JsProviderInstance.kt` |
