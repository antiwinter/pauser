...
public class MediaStateDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfMediaStateEntity = object : EntityInsertAdapter<MediaStateEntity>() {
      protected override fun createQuery(): String =
          "INSERT OR REPLACE INTO `media_state` (`providerType`,`sourceId`,`itemId`,`positionMs`,`playbackSpeed`,`isFavorite`,`title`,`type`,`coverCachePath`,`selectedSubtitleTrackId`,`selectedAudioTrackId`,`updatedAtEpochMs`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"

      protected override fun bind(statement: SQLiteStatement, entity: MediaStateEntity) {
        statement.bindText(1, entity.providerType)
  ...
  public override suspend fun `get`(
    ...
    val _sql: String =
        "SELECT * FROM media_state WHERE providerType = ? AND sourceId = ? AND itemId = ? LIMIT 1"
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          _result =
              MediaStateEntity(_tmpProviderType,_tmpSourceId,_tmpItemId,_tmpPositionMs,_tmpPlaybackSpeed,_tmpIsFavorite,_tmpTitle,_tmpType,_tmpCoverCachePath,_tmpSelectedSubtitleTrackId,_tmpSelectedAudioTrackId,_tmpUpdatedAtEpochMs)
        } else {
          _result = null
        }
        _result
  ...
  public override fun observeForSource(providerType: String, sourceId: String):
      Flow<List<MediaStateEntity>> {
    val _sql: String =
        "SELECT * FROM media_state WHERE providerType = ? AND sourceId = ? ORDER BY updatedAtEpochMs DESC"
  ...
  public override suspend fun updatePosition(
    ...
    val _sql: String =
        "UPDATE media_state SET positionMs = ?, updatedAtEpochMs = ? WHERE providerType = ? AND sourceId = ? AND itemId = ?"
  ...
  public override suspend fun updateSpeed(
    ...
    val _sql: String =
        "UPDATE media_state SET playbackSpeed = ?, updatedAtEpochMs = ? WHERE providerType = ? AND sourceId = ? AND itemId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun updateFavorite(
    ...
    val _sql: String =
        "UPDATE media_state SET isFavorite = ?, title = ?, type = ?, updatedAtEpochMs = ? WHERE providerType = ? AND sourceId = ? AND itemId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun updateCoverCache(
    ...
    val _sql: String =
        "UPDATE media_state SET coverCachePath = ?, updatedAtEpochMs = ? WHERE providerType = ? AND sourceId = ? AND itemId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun updateSubtitleTrack(
    ...
    val _sql: String =
        "UPDATE media_state SET selectedSubtitleTrackId = ?, updatedAtEpochMs = ? WHERE providerType = ? AND sourceId = ? AND itemId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun updateAudioTrack(
    ...
    val _sql: String =
        "UPDATE media_state SET selectedAudioTrackId = ?, updatedAtEpochMs = ? WHERE providerType = ? AND sourceId = ? AND itemId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun delete(
    ...
    val _sql: String =
        "DELETE FROM media_state WHERE providerType = ? AND sourceId = ? AND itemId = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
