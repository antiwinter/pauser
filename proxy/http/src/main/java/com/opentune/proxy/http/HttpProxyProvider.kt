package com.opentune.proxy.http

import com.opentune.provider.ProviderFieldKind
import com.opentune.provider.ProviderFieldSpec
import com.opentune.provider.ValidationResult
import com.opentune.proxy.ProxyProvider
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import kotlinx.serialization.json.Json
import okhttp3.Authenticator
import okhttp3.Credentials
import okhttp3.OkHttpClient
import okhttp3.Request
import java.net.InetSocketAddress
import java.net.Proxy

class HttpProxyProvider : ProxyProvider {

    override val proxyType: String = "http"

    override fun getFieldsSpec(): List<ProviderFieldSpec> = listOf(
        ProviderFieldSpec(
            id = "host",
            labelKey = "fld_proxy_host",
            kind = ProviderFieldKind.SingleLineText,
            required = true,
            order = 0,
            placeholderKey = "ph_proxy_host",
        ),
        ProviderFieldSpec(
            id = "port",
            labelKey = "fld_proxy_port",
            kind = ProviderFieldKind.SingleLineText,
            required = true,
            order = 1,
            placeholderKey = "ph_proxy_port",
        ),
        ProviderFieldSpec(
            id = "username",
            labelKey = "fld_account_username",
            kind = ProviderFieldKind.SingleLineText,
            required = false,
            order = 2,
        ),
        ProviderFieldSpec(
            id = "password",
            labelKey = "fld_account_password",
            kind = ProviderFieldKind.Password,
            required = false,
            sensitive = true,
            order = 3,
        ),
    )

    override suspend fun validateFields(values: Map<String, String>): ValidationResult =
        withContext(Dispatchers.IO) {
            try {
                val host = values["host"]?.trim().orEmpty()
                val port = values["port"]?.trim()?.toIntOrNull()
                    ?: return@withContext ValidationResult.Error("Port must be a number")
                if (host.isEmpty()) return@withContext ValidationResult.Error("Host is required")
                if (port !in 1..65535) return@withContext ValidationResult.Error("Port must be between 1 and 65535")

                val client = createClient(values)
                val request = Request.Builder().url("http://$host:$port").build()
                client.newCall(request).execute().use { }

                val displayName = "$host:$port"
                ValidationResult.Success(
                    hash = "$host:$port",
                    name = displayName,
                    fields = mapOf(
                        "host" to host,
                        "port" to port.toString(),
                        "username" to (values["username"]?.trim() ?: ""),
                        "password" to (values["password"] ?: ""),
                    ),
                )
            } catch (e: Exception) {
                ValidationResult.Error(e.message ?: "Proxy validation failed")
            }
        }

    override fun createClient(values: Map<String, String>): OkHttpClient {
        val host = values["host"]?.trim() ?: error("Missing proxy host")
        val port = values["port"]?.trim()?.toIntOrNull() ?: error("Missing proxy port")
        val username = values["username"]?.trim() ?: ""
        val password = values["password"] ?: ""

        val proxy = Proxy(Proxy.Type.HTTP, InetSocketAddress(host, port))
        return OkHttpClient.Builder()
            .proxy(proxy)
            .apply {
                if (username.isNotEmpty()) {
                    proxyAuthenticator(Authenticator { _, response ->
                        if (response.request.header("Proxy-Authorization") != null) return@Authenticator null
                        response.request.newBuilder()
                            .header("Proxy-Authorization", Credentials.basic(username, password))
                            .build()
                    })
                }
            }
            .build()
    }
}
