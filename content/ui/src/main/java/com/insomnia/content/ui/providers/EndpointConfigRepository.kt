package com.insomnia.content.ui.providers
...
import com.insomnia.core.form.SubmitResult
...
object EndpointConfigRepository {
    ...
    private fun identityKeys(protocol: String): Set<String> {
        return InsomniaProviderRegistryHolder.get().provider(protocol)
            .getFieldsSpec()
            .filter { it.identity }
            .map { it.id }
            .toSet()
    ...
    suspend fun loadEditFields(protocol: String, endpointId: String): Map<String, String> =
        withContext(Dispatchers.IO) {
            ...
            val spec = InsomniaProviderRegistryHolder.get().provider(protocol).getFieldsSpec()
