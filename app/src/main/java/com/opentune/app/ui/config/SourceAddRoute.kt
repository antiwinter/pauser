...
private const val LOG_TAG = "OpenTuneSourceAdd"
...
fun SourceAddRoute(
    ...
    LaunchedEffect(protocol) {
        val initial = SourceConfigRepository.loadAddDraft(protocol, app)
        ...
        snapshotFlow { values }
            ...
            .collect { v ->
                withContext(Dispatchers.IO) {
                    SourceConfigRepository.saveAddDraft(protocol, app, v)
                }
            }
    }
    ...
    Column(
        ...
        Column(
            ...
            Text(stringResource(R.string.source_add_hint_autosave))
            fields.forEach { spec ->
        ...
        Button(
                ...
                scope.launch {
                    ...
                    try {
                        val result = withContext(Dispatchers.IO) {
                            SourceConfigRepository.submitAdd(protocol, values, app)
                        }
                        when (result) {
                            is SubmitResult.Success -> {
                                withContext(Dispatchers.IO) {
                                    SourceConfigRepository.clearAddDraft(protocol, app)
                                }
                                onDone()
                            }
            ...
            Row(
                ...
                Text(stringResource(R.string.source_add_primary))
            }
        }
        error?.let {
            Text(
