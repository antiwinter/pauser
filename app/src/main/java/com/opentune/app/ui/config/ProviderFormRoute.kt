...
fun ProviderFormRoute(
    ...
    val proxies by app.storageBindings.proxyDao.observeAll()
        ...
        if (isAdd && entityType == FormEntityType.ENDPOINT) {
            ...
            val initial = withContext(Dispatchers.IO) {
                when (entityType) {
                    ...
                    FormEntityType.PROXY -> ProxyRepository.loadEditFields(protocol, app, existingId!!)
                }
            }
            values = fields.associate { it.id to (initial[it.id] ?: "") }
            if (isEndpoint) {
                selectedProxyId = withContext(Dispatchers.IO) {
                    EndpointConfigRepository.loadEditProxyId(existingId!!, app)
                }
            }
            loaded = true
        }
                        ...
                        val result: SubmitResult = withContext(Dispatchers.IO) {
                            when {
                                entityType == FormEntityType.ENDPOINT && isAdd ->
                                    EndpointConfigRepository.submitAdd(protocol, values, selectedProxyId, app)
                                entityType == FormEntityType.ENDPOINT && !isAdd ->
                                    EndpointConfigRepository.submitEdit(protocol, existingId!!, values, selectedProxyId, app)
                                entityType == FormEntityType.PROXY && isAdd ->
                                    ProxyRepository.submitAdd(protocol, values, app)
                                else ->
                                    ProxyRepository.submitEdit(protocol, existingId!!, values, app)
                            }
                        }
                        when (result) {
