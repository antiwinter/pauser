...
import com.opentune.app.ui.config.ProxyManageRoute
...
object Routes {
    ...
    const val PROXY_MANAGE = "proxy_manage"
    const val PROXY_ADD = "proxy_add/{proxyType}"
    const val PROXY_EDIT = "proxy_edit/{proxyType}/{proxyConfigId}"
    ...
    fun proxyEdit(proxyType: String, proxyConfigId: String) =
        "proxy_edit/$proxyType/${URLEncoder.encode(proxyConfigId, UrlCharset)}"
...
fun OpenTuneNavHost() {
    ...
    NavHost(navController = nav, startDestination = Routes.HOME) {
        ...
        composable(Routes.PROXY_MANAGE) {
            ProxyManageRoute(
                onAddProxy = { proxyType -> nav.navigate(Routes.proxyAdd(proxyType)) },
                onEditProxy = { proxyType, proxyConfigId -> nav.navigate(Routes.proxyEdit(proxyType, proxyConfigId)) },
        ...
        composable(
            ...
            listOf(
                ...
                navArgument("proxyConfigId") { type = NavType.StringType },
            ),
        ) {
            val proxyType = it.arguments!!.getString("proxyType")!!
            val proxyConfigId = it.arguments!!.getString("proxyConfigId")!!
            ProviderFormRoute(
                entityType = FormEntityType.PROXY,
                protocol = proxyType,
                existingId = proxyConfigId,
                onDone = { nav.popBackStack() },
            )
        }
        composable(
        ...
        composable(Routes.SETTINGS) {
            SettingsScreen(
                app = app,
                onBack = { nav.popBackStack() },
                onManageProxies = { nav.navigate(Routes.PROXY_MANAGE) },
            )
        }
        composable(
            Routes.IMAGE_VIEWER,
